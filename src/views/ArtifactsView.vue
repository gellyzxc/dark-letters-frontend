    ...
    <div class="main">
      <div class="row opened-cards">
        ...
        >
          <drag-drop-component
            ...
            @drag-start="showRemoveSlot($event)"
            @drag-end="hideRemoveSlot($event)"
          >
            <img
              v-if="slot.card"
              :src="slot.card.imageUrl"
              :alt="slot.card.name"
              @click="handleCardClick(slot)"
      ...
      <div class="row closed-cards">
        <drag-drop-component
          class="remove-zone"
          group-id="artifacts"
          :class="{ show: removeSlot }"
          @item-dropped="handleCardRemoved($event)"
        >
          <div></div>
        </drag-drop-component>
        <div
          v-for="card in [...availableCards, ...closedCards]"
          :key="card.id"
        ...
        >
          <drag-drop-component
            ...
            @item-dropped="() => {}"
            :class="{ 'flipped-card-drag': card.isFlipped }"
          >
  ...
  methods: {
    ...
    async loadArtifacts() {
      try {
        ...
        const openedCount =
          equippedArtifacts.length + inventoryArtifacts.length;
    ...
    async handleCardDropped(targetSlotId, { droppedData, currentData }) {
      const targetSlotIndex = this.cardSlots.findIndex(
        (s) => s.id === targetSlotId,
      );
      ...
      try {
        if (fromSlotIndex !== -1) {
          if (currentData) {
            ...
            this.cardSlots[targetSlotIndex].card =
              this.cardSlots[fromSlotIndex].card;
...
.card-simple {
  ...
  box-shadow:
    $shadow-md,
    0 0 10px rgba($color-brown-light, 0.15),
