  ...
  <div class="node-editor">
    <div class="editor-header">
      ...
      <div class="controls">
        ...
        <button @click="clearBackground" class="btn-reset" v-if="backgroundImage">Clear BG</button>
      </div>
    </div>
    <div class="editor-container">
      <div ref="canvasContainer" class="canvas-container">
        <svg ref="svgCanvas" class="svg-canvas" @mousedown="handleCanvasMouseDown" @mousemove="handleCanvasMouseMove"
          @mouseup="handleCanvasMouseUp" @wheel="handleWheel">
          <g :transform="`translate(${viewOffset.x}, ${viewOffset.y}) scale(${zoom})`">
            <defs>
              <pattern id="grid" width="50" height="50" patternUnits="userSpaceOnUse">
                <rect width="50" height="50" fill="none" stroke="#2a2a2a" stroke-width="0.5" />
              </pattern>
            </defs>
            <rect x="-5000" y="-5000" width="10000" height="10000" fill="url(#grid)" />
            <image v-if="backgroundImage" :href="backgroundImage" :x="bgPosition.x" :y="bgPosition.y"
              :width="bgSize.width" :height="bgSize.height" :opacity="bgOpacity" class="background-image" />
            <line x1="-5000" y1="0" x2="5000" y2="0" stroke="#3a3a3a" stroke-width="2" />
            <line x1="0" y1="-5000" x2="0" y2="5000" stroke="#3a3a3a" stroke-width="2" />
            <g class="links">
              <line v-for="link in links" :key="`${link.from}-${link.to}`" :x1="getNodeById(link.from)?.x || 0"
                :y1="getNodeById(link.from)?.y || 0" :x2="getNodeById(link.to)?.x || 0"
                :y2="getNodeById(link.to)?.y || 0" stroke="#4a3828" stroke-width="3" class="link-line" />
            </g>
            <g class="nodes">
              <g v-for="node in nodes" :key="node.uid" :transform="`translate(${node.x}, ${node.y})`"
                @mousedown.stop="handleNodeMouseDown($event, node)" class="node"
                :class="{ selected: selectedNode?.uid === node.uid }">
                <circle r="25" :fill="node.uid === 0 ? '#d4a574' : '#4a3828'"
                  :stroke="selectedNode?.uid === node.uid ? '#ffd7a3' : '#8a7a6a'" stroke-width="2"
                  class="node-circle" />
                <text text-anchor="middle" dy="5" fill="white" font-size="14" font-weight="bold" pointer-events="none">
                  {{ node.uid }}
                </text>
              </g>
            </g>
      ...
      <div class="sidebar">
        <div class="sidebar-section">
          ...
          <div v-if="selectedNode" class="node-info">
            ...
            <div class="info-row">
              <label>Name:</label>
              <input v-model="selectedNode.name" type="text" class="input-text" />
            </div>
            <div class="info-row">
              <label>Value:</label>
              <textarea v-model="selectedNode.value" class="input-textarea"></textarea>
            </div>
            <div class="info-row">
              <label>X:</label>
              <input v-model.number="selectedNode.x" type="number" class="input-number" />
            </div>
            <div class="info-row">
              <label>Y:</label>
              <input v-model.number="selectedNode.y" type="number" class="input-number" />
            </div>
            <div class="info-row">
              <label>Image:</label>
              <input v-model="selectedNode.image" type="text" class="input-text" />
            </div>
          </div>
        ...
        <div class="sidebar-section">
          ...
          <button @click="deleteNode" class="btn-action" :disabled="!selectedNode || selectedNode.uid === 0">Delete
            Node</button>
        ...
        <div class="sidebar-section" v-if="backgroundImage">
          ...
          <div class="bg-controls">
            <div class="info-row">
              <label>Opacity:</label>
              <input v-model.number="bgOpacity" type="range" min="0" max="1" step="0.1" class="input-range" />
              <span>{{ (bgOpacity * 100).toFixed(0) }}%</span>
            </div>
            <div class="info-row">
              <label>Width:</label>
              <input v-model.number="bgSize.width" type="number" class="input-number" />
            </div>
            <div class="info-row">
              <label>Height:</label>
              <input v-model.number="bgSize.height" type="number" class="input-number" />
            </div>
            <div class="info-row">
              <label>X Position:</label>
              <input v-model.number="bgPosition.x" type="number" class="input-number" />
            </div>
            <div class="info-row">
              <label>Y Position:</label>
              <input v-model.number="bgPosition.y" type="number" class="input-number" />
            </div>
            <button @click="centerBackground" class="btn-action">Center Background</button>
            <button @click="lockAspectRatio = !lockAspectRatio" class="btn-action">
    ...
    <div v-if="showExportModal" class="modal-overlay" @click="showExportModal = false">
      <div class="modal-content" @click.stop>
        <h2>Exported JSON</h2>
        <textarea v-model="exportedJSON" readonly class="json-output"></textarea>
        <div class="modal-actions">
          <button @click="copyToClipboard" class="btn-copy">Copy to Clipboard</button>
          <button @click="showExportModal = false" class="btn-close">Close</button>
        </div>
      </div>
    </div>
    <div v-if="showImportModal" class="modal-overlay" @click="showImportModal = false">
      <div class="modal-content" @click.stop>
        <h2>Import JSON</h2>
        <textarea v-model="importJSON" class="json-output" placeholder="Paste JSON here..."></textarea>
        <div class="modal-actions">
          <button @click="performImport" class="btn-copy">Import</button>
          <button @click="showImportModal = false" class="btn-close">Cancel</button>
        </div>
      </div>
    </div>
  </div>
...
function addLink() {
  ...
  const linkExists = links.value.some(l =>
    (l.from === fromUid && l.to === toUid) || (l.from === toUid && l.to === fromUid)
  )
  if (linkExists) {
...
function deleteLink() {
  ...
  if (connectedLinks.length === 0) {
    alert('No links connected to this node')
    return
  }
  const linkDescription = connectedLinks.map(({ link, index }) =>
    `${index}: ${link.from} -> ${link.to}`
  ).join('\n')
  const indexStr = prompt(`Select link to delete:\n${linkDescription}\n\nEnter index:`)
  if (!indexStr) return
...
function handleKeyDown(event) {
  if (event.key === 'Delete' && selectedNode.value && selectedNode.value.uid !== 0) {
    deleteNode()
  }
}
const bgSizeHandler = {
