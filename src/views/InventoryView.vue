<template>
  <div class="page inventory-view">
    <transition name="fade">
      <div
        v-if="showItemTooltip && hoveredItem"
        class="item-tooltip"
    ...
    <div class="main">
      ...
      <div class="content">
        <div class="stats-left">
          <span class="heading" v-if="character">{{
            character.race?.name
          }}</span>
          <span class="heading" v-if="character">{{
          ...
          <span v-if="character"
            ><span class="value">+{{ character.intellect }}</span>
            intelligence</span
          >
          <span v-if="character"
            ><span class="value">+{{ character.strength }}</span> strength</span
          >
          <span v-if="character"
            ><span class="value">+{{ character.agility }}</span> agility</span
          >
          <span v-if="character" class="mt-1"
            ><span class="value">+{{ character.damage }}</span> damage</span
          >
          <span v-if="character"
            ><span class="value"
              >+{{ Math.round(character.critical_hit_chance * 100) }}%</span
            >
            krit chance</span
          >
          <span v-if="character"
            ><span class="value"
              >+{{ Math.round(character.critical_multiplier) }}%</span
            >
            krit damage</span
          >
          <span v-if="character"
            ><span class="value">+{{ character.defence }}</span> defence</span
          >
          <span v-if="character"
            ><span class="value">+{{ character.health_points }}</span> health
            points</span
          >
          <span v-if="character"
            ><span class="value"
              >+{{ Math.round(character.block_chance * 100) }}%</span
            >
            block chance</span
          >
          <span v-if="character"
            ><span class="value">+{{ character.mana }}</span> mana</span
          >
          <span v-if="character"
            ><span class="value"
              >+{{ Math.round(character.mana_chance * 100) }}%</span
            >
            mana chance</span
          >
        </div>
        <div class="stats-bottom">
          ...
          <div class="col">
            <span v-if="character"
              >{{ character.experience }}/{{ character.required_exp }} exp</span
            >
        ...
        <div class="sword">
          <drag-drop-component
            ...
            group-id="inventory"
            :item-data="equippedSword"
            @item-dropped="onEquipmentDropped('sword', $event)"
            @drag-start="onDragStartHandler"
            @drag-end="onDragEndHandler"
          >
            <div
              v-if="equippedSword"
              class="item"
              @mouseenter="onItemHover(equippedSword, $event)"
              @mouseleave="onItemLeave"
            >
              <img
                v-if="equippedSword.photo"
                style="max-height: 182px"
                :src="getImageUrl(equippedSword.photo)"
                :alt="equippedSword.name"
                class="item-image"
              />
        ...
        <div class="cloth">
          ...
          >
            <div
              v-if="equippedCloth"
              class="item"
              @mouseenter="onItemHover(equippedCloth, $event)"
              @mouseleave="onItemLeave"
            >
              <img
                v-if="equippedCloth.photo"
                :src="getImageUrl(equippedCloth.photo)"
                :alt="equippedCloth.name"
                class="item-image"
              />
        ...
        <div class="amulet">
          ...
          >
            <div
              v-if="equippedAmulet"
              class="item"
              @mouseenter="onItemHover(equippedAmulet, $event)"
              @mouseleave="onItemLeave"
            >
              <img
                v-if="equippedAmulet.photo"
                :src="getImageUrl(equippedAmulet.photo)"
                :alt="equippedAmulet.name"
                class="item-image"
              />
        ...
        <div
          class="inventory"
          @dragover.prevent="onInventoryDragOver"
          @dragleave.prevent="onInventoryDragLeave"
        >
          <drag-drop-component
            ...
            :grid-cols="8"
            :grid-rows="8"
            :occupied-slots="inventorySlots.map((s) => s.item)"
            @item-dropped="onItemDropped('inventory', index, $event)"
            @drag-start="onDragStartHandler"
            @drag-end="onDragEndHandler"
          >
            <div
              v-if="slot.item && isFirstSlotOfItem(index)"
              class="item"
              :style="getItemStyle(slot.item, index)"
              @mouseenter="onItemHover(slot.item, $event)"
              @mouseleave="onItemLeave"
            >
              <img
                v-if="slot.item.photo"
                :src="getImageUrl(slot.item.photo)"
                :alt="slot.item.name"
                class="item-image"
              />
...
export default {
  ...
  methods: {
    ...
    isFirstSlotOfItem(index) {
      ...
      const firstIndex = this.inventorySlots.findIndex(
    ...
    async loadInventory() {
      try {
        ...
        inventory.forEach((apiItem) => {
          const transformedItem = transformApiItem(apiItem);
          const slotIndex = positionToIndex(
            transformedItem.position.x,
            transformedItem.position.y,
          );
    ...
    async updateItemPosition(item, newIndex) {
      try {
        ...
        console.log(
          `Updated item ${item.name} position to (${position.x}, ${position.y})`,
        );
      } catch (error) {
    ...
    async onEquipmentDropped(slotType, { droppedData, currentData }) {
      ...
      const sourceInvIndex = this.inventorySlots.findIndex(
        (s) => s.item && s.item.id === droppedData.id,
      );
      if (sourceInvIndex !== -1) {
        ...
        if (oldEquipped) {
          ...
          await this.inventoryStore.updateItem(oldEquipped.id, {
            is_equipped: false,
          });
        ...
        await this.inventoryStore.updateItem(droppedData.id, {
          is_equipped: true,
        });
    ...
    async onSellItem({ droppedData }) {
      ...
      const confirmSell = confirm(
        `Sell ${droppedData.name} for ${droppedData.price || 0} gold?`,
      );
      if (!confirmSell) {
        ...
        for (const sourceSlot of sourceSlots) {
          ...
          if (this[sourceProp] && this[sourceProp].id === droppedData?.id) {
            ...
            if (targetItem) {
              ...
              await this.inventoryStore.updateItem(targetItem.id, {
                is_equipped: true,
              });
            } else {
