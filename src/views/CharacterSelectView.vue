<template>
  <div class="character-select">
    <div class="header">
      ...
      <span v-if="currentStep === 'class' && selectedRace" class="back-button" @click="goBackToRace">← Back to
        Race</span>
    </div>
    <Swiper v-if="currentStep === 'race'" :loop="true" :navigation="true" :modules="modules" class="swiper"
      ...
      <SwiperSlide v-for="race in races" :key="race.id" class="slide">
        <div class="slide-content" :class="{ zoom: isZooming }">
          <div class="character-image">
            <img :src="mageImage" :alt="race.name" />
          </div>
    ...
    <div v-if="currentStep === 'race'" class="select-button" @click="selectCurrentRace()">
      <span>Select Race</span>
    </div>
    <Swiper v-if="currentStep === 'class'" :loop="true" :navigation="true" :modules="modules" class="swiper"
      @swiper="onSwiperInit" @slideChange="onSlideChange">
      <SwiperSlide v-for="character in characters" :key="character.name" class="slide">
    ...
    <div v-if="currentStep === 'class'" class="select-button" @click="selectCurrentCharacter()">
      <span>Select Class</span>
    </div>
    <ModalComponent :open="showRaceConfirmModal" @willDismiss="showRaceConfirmModal = false">
      <FrameComponent type="generic-card-horizontal" class="modal-frame">
        <div class="confirm-content">
          <h3>Confirm Race Selection</h3>
          <p>Are you sure you want to choose <strong>{{ tempSelectedRace?.name }}</strong> as your race?</p>
          <div class="buttons">
    ...
    <ModalComponent :open="showConfirmModal" @willDismiss="showConfirmModal = false">
      <FrameComponent type="generic-card-horizontal" class="modal-frame">
        <div class="confirm-content">
          <h3>Confirm Class Selection</h3>
          <p>Are you sure you want to choose <strong>{{ selectedCharacter?.name }}</strong> as your class?</p>
          <div class="buttons">
...
.slide {
  ...
  background: linear-gradient(to bottom, rgba($color-bg-base, 0.8), rgba($color-bg-base, 0.95));
}
...
.select-button {
  ...
  background: linear-gradient(135deg, rgba($color-gold-primary, 0.15) 0%, rgba($color-gold-primary, 0.25) 100%);
  border: 2px solid rgba($color-gold-primary, 0.6);
  ...
  &:hover {
    background: linear-gradient(135deg, rgba($color-gold-primary, 0.25) 0%, rgba($color-gold-primary, 0.35) 100%);
