<template>
  <div class="page inventory-view">
    ...
    <transition name="fade">
      <div
        v-if="showReforgedTooltip && reforgedItem"
        class="item-tooltip reforged-tooltip"
    ...
    <div class="main">
      ...
      <div class="content">
        <div class="forge">
          ...
          >
            <div
              v-if="forgeItem"
              class="item"
              @mouseenter="onItemHover(forgeItem, $event)"
              @mouseleave="onItemLeave"
            >
              <img
                v-if="forgeItem.photo"
                style="max-height: 110px"
                :src="getImageUrl(forgeItem.photo)"
                :alt="forgeItem.name"
                class="item-image"
              />
        ...
        <div
          class="reforge"
          @click="onReforgeClick"
          :class="{ opening: openingReforge }"
        ></div>
        <transition name="item-drop">
          <div
            v-if="reforgeItem"
            class="dropped-item-container"
            @click="reforgeItem = null"
          >
  ...
  methods: {
    ...
    async acceptReforge() {
      ...
      try {
        ...
        await this.inventoryStore.updateItem(this.reforgedItem.id, {
          stats: statsUpdate,
        });
        if (this.totalGoldSpent > 0) {
          const { useCharacterStore } = await import("@/stores/character");
          const characterStore = useCharacterStore();
          await characterStore.updateCharacter(this.character.id, {
        ...
        this.$toast.show("Failed to save reforged item");
      }
    },
