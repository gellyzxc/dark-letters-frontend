  ...
  <div class="item-info-card" :class="cardClass">
    ...
    <div v-if="variant === 'reward'" class="reward-layout">
      ...
      <template v-if="itemType === 'gold'">
        <div class="reward-left">
          <div class="item-name-reward">
            ...
            <span class="description-text">&nbsp;The King's Legendary Offering</span>
          </div>
        </div>
        
        <div class="reward-center">
          <div class="item-icon-reward gold-icon">
      ...
      <template v-else-if="itemType === 'artifact'">
        ...
        <div class="reward-center">
          <div class="item-icon-reward with-glow">
            <img v-if="item.photo" :src="getImageUrl(item.photo)" :alt="item.name" />
            <span v-else>{{ item.icon || "📦" }}</span>
          </div>
        </div>
      ...
      <template v-else>
        <div class="reward-left">
          <div class="item-name-reward">
            <span class="name-text">{{ item.name }}</span>
            <span class="description-text" v-if="item.description">&nbsp;{{ item.description }}</span>
          </div>
        </div>
        ...
        <div class="reward-right">
          <div class="properties-title" v-if="item.stats && item.stats.length > 0">Properties</div>
          <div class="item-bonuses-reward" v-if="item.stats && item.stats.length > 0">
            <div v-for="(stat, index) in item.stats" :key="index" class="bonus-line-reward">
              <span>{{ formatStatLabel(stat.label) }}</span>
            </div>
          </div>
        </div>
      ...
      <div class="item-bonuses" v-if="item.stats && item.stats.length > 0">
        ...
        >
          <div style="display: contents" v-if="!reforge">
            <span class="bonus-label" style="white-space: nowrap; text-align: right;">{{ formatStatLabel(stat.label) }}</span>
          </div>
          <div style="display: contents" v-else>
            <span class="bonus-value" style="white-space: nowrap; text-align: right;">{{ formatStatLabel(stat.label.replace(new RegExp('[0-9]+'), stat.value).replace('%%', '%')) }}</span>
          </div>
        </div>
        <span
          style="
...
.item-info-card {
  ...
  &.variant-reward {
    ...
    // Actions slot positioned at bottom left
